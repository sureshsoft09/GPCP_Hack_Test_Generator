...
Provides Model Context Protocol (MCP) interface for Firestore operations
Following FastMCP architecture pattern like jira_mcp
"""
...
async def get_project_epics(project_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error(f"Error fetching epics for project {project_id}: {e}")
        return {
            "success": False,
            "error": str(e)
        }
...
async def get_epic_features(project_id: str, epic_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error(f"Error fetching features for epic {epic_id}: {e}")
        return {
            "success": False,
            "error": str(e)
        }
...
async def get_feature_use_cases(project_id: str, epic_id: str, feature_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error(f"Error fetching use cases for feature {feature_id}: {e}")
        return {
            "success": False,
            "error": str(e)
        }
